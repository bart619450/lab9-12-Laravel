    ...
    public function index(Request $request)
{
    ...
    if ($request->has('search') && $request->search != '') {
        ...
        $query->where('tytul', 'like', "%$search%")
              ->orWhere('id', $search)
              ->orWhereHas('autorzy', function ($q) use ($search) {
                  $q->where('imie', 'like', "%$search%")
                    ->orWhere('nazwisko', 'like', "%$search%");
              });
    }

    $filmy = $query->paginate(10);

    return view('film.index', compact('filmy'));
}
public function user_index(Request $request)
{
    $filmy = Film::where('user_id', auth()->id())  // Filtruj filmy po user_id
                ->when($request->search, function ($query) use ($request) {
                    return $query->where('tytul', 'like', '%' . $request->search . '%');
                })
                ->paginate(10);  // Paginacja
    
    return view('film.user_index', compact('filmy'));
}
