...
class TypFilmuController extends Controller
{
    ...
    {
        $typyfilmow = TypFilmu::with('filmy')->paginate(10);
        return view('typfilmu.index', compact('typyfilmow'));
    }

    public function user_index(Request $request)
{
    $typyfilmow = TypFilmu::where('user_id', auth()->id())  // Filtruj filmy po user_id
                ->when($request->search, function ($query) use ($request) {
                    return $query->where('nazwa', 'like', '%' . $request->search . '%');
                })
                ->paginate(10);  // Paginacja
    
    return view('typfilmu.user_index', compact('typyfilmow'));
}
    ...
    public function store(Request $request)
    {
        ...
        $typfilmu = new TypFilmu();
        $typfilmu->nazwa = $request->nazwa;
        $typfilmu->user_id = auth()->user()->id; 
        $typfilmu->save();
