    ...
    <form method="POST" action="{{ route('aktor.store') }}">
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control" id="imie" name="imie" value="{{ old('imie', $aktor->imie ?? '') }}" required>
            </div>

            <!-- Data premiery -->
            <div class="col-md-6 mb-3">
                <label for="nazwisko" class="form-label">Nazwisko</label>
                <input type="text" class="form-control" id="nazwisko" name="nazwisko" value="{{ old('nazwisko', $aktor->nazwisko ?? '') }}">
            </div>
        </div>
            ...
            <div class="col-md-6 mb-3">
                ...
                <select multiple id="filmy" name="filmy[]" class="form-control">
                    @foreach($filmy as $film)
                        <option value="{{ $film->id }}" {{ in_array($film->id, old('filmy', $aktor->filmy->pluck('id')->toArray() ?? [])) ? 'selected' : '' }}>
                            {{ $film->tytul }}
                        </option>
                    @endforeach
                </select>
